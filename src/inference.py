...
#     MODELS_DIR / "tfidf_logreg.joblib",  # keep legacy filename as fallback
# ]
...
# # Transformer
# TRANS_DIR_ENV = os.getenv("TRANSFORMER_MODEL_DIR", "").strip()
...
#     cands = [p for p in MODELS_DIR.glob("finetuned_*") if p.is_dir()]
#     return max(cands, default=None, key=lambda p: p.stat().st_mtime)
...
#     model_dir = Path(TRANS_DIR_ENV) if TRANS_DIR_ENV and Path(TRANS_DIR_ENV).is_dir() else _latest_finetuned_dir()
#     if not model_dir or not model_dir.exists():
#         raise FileNotFoundError(
#             "No finetuned_* transformer directory found in models/. "
#             "Set TRANSFORMER_MODEL_DIR or train a model."
#         )
...
#             except Exception:
#                 pass
#         # decode numeric class ids to names via label encoder
#         try:
