...
def main() -> None:
    ...
    # Save model (BOTH names so inference can find it automatically)
    MODELS_DIR.mkdir(parents=True, exist_ok=True)
    robust_path = MODELS_DIR / "robust_baseline_model.joblib"
    joblib.dump(pipeline, robust_path)
    print(f"[model] Final model saved at {robust_path}")

    # NEW: also save under classic name used by inference
    classic_path = MODELS_DIR / "tfidf_logreg.joblib"
    joblib.dump(pipeline, classic_path)
    print(f"[model] Also saved model at {classic_path}")
